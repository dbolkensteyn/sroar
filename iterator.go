...
func (itr *IteratorOpt) skipEmpty() {
	itr.keyIdx++
	itr.key = itr.r.keys.key(itr.keyIdx)
	itr.arrIdx = -1
	off := itr.r.keys.val(itr.keyIdx)
	con := itr.r.getContainer(off)
	c := getCardinality(con)
	if c > 0 {
		switch con[indexType] {
		case typeArray:
			itr.arr = array(con).all()
		case typeBitmap:
			itr.arr = bitmap(con).all()
		}
		return
	}

	skipped := false
	for c == 0 && itr.keyIdx < itr.numKeys {
		itr.keyIdx++
		itr.key = itr.r.keys.key(itr.keyIdx)
		off = itr.r.keys.val(itr.keyIdx)
		con = itr.r.getContainer(off)
...
func (itr *IteratorOpt) HasNext() bool {
	if !itr.reverse {
		// fmt.Printf("hasNext called on keyIdx: %d\n", itr.keyIdx)
		if itr.keyIdx == itr.numKeys {
			return false
		}
